...
MPlayerSlave.prototype.start = function (cmdLineOptions, flags) {
    ...
    this.child.on('exit', function (code, sig) {
        if (code !== null && sig === null) {
            ...
            this.emit('closed');
            console.log("Slave instance has been killed.");
        }
    }.bind(this));
    ...
    console.log('Slave instance started...');
    console.log('Command FIFO at: ' + this.cmdFifoPath);
};
...
MPlayerSlave.prototype.sendCommand = function(commandStr) {
	if (this.child != null) {
		...
		console.log('Sent the following command(s) to mplayer: \n' + commandStr);

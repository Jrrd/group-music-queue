...
var app = express()
,   musicQueue = new ObjectQueue(properties.QUEUE_MAX_SIZE)
,	queuePlayer = new LocalQueuePlayer(musicQueue)
...
app.get('/current-playlist/:index?', function(req, res) {
	var index = req.params.index;

	try {
		if (typeof index === 'undefined') {

			logger.info("Sending back list of songs of: " + musicQueue.getArray().length);
			res.send(200, musicQueue.getArray());

		}
		else {

			var item = musicQueue.getArray()[index];

			if (item) {
				res.send(200, item);
			}
			else {
				res.send(400, 'Resource not available');
			}

		}
	}
	catch(reqEx) {
		res.send(400, reqEx.message || "Error retrieving list");
		logger.error("Error queuing => " + reqEx.toString());
	}

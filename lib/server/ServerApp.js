...
app.post('/indexfile', function(req, res) {
	...
	try {
		console.log('attempting to add index file to server: ', audioReq);
		indexPool.add(indexFile);
		res.send(204);
	}
	catch (queueErr) {
		res.send(400, queueErr.message || "Error storing index file");
	}
});
...
app.delete('/indexfile', function(req, res) {
	...
	try {
		console.log('attempting to remove index file to server: ', audioReq);
		indexPool.remove(clientID);
		res.send(204);
	}
	catch (queueErr) {
		res.send(400, queueErr.message || "Error removing index file");
	}
});
